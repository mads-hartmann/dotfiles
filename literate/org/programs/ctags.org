...
I don't use ~ctags~ anymore really. It's a remnant from my Emacs days.
I keep the configuration around though just in case.

#+CAPTION: ~~/.ctags.cnf~
#+BEGIN_SRC txt :tangle ../../../.home/.ctags.cnf
--exclude=.git
--exclude=.hg
--exclude=log
--exclude=tmp
...
--langmap=js:.js
--langmap=js:+.jsx
--regex-js=/^[ \t]*export (default)?[ \t]*[ \t]+([a-zA-Z0-9_]+)/\2/c,classes/
--regex-js=/^[ \t]*export (default)?[ \t]*class[ \t]+([a-zA-Z0-9_]+)/\2/c,classes/
--regex-js=/^[ \t]*export (default)?[ \t]*function[ \t]+([a-zA-Z0-9_]+)/\2/f,functions/
--regex-js=/^[ \t]*export (default)?[ \t]*const[ \t]+([a-zA-Z0-9_]+)/\2/v,variables/
